...
class CursoDetail(Resource):
    ...
    def get(self, id):
        curso = curso_service.listar_curso_id(id=id)
        if curso is None:
    ...
    def put(self, id):
        curso_bd = curso_service.listar_curso_id(id=id)
        if curso_bd is None:
            return make_response(jsonify('Curso não encontrado'), 404)
        cs = curso_schema.CursoSchema()
        validate = cs.validate(request.json)
        if validate:
            ...
            curso_atualizado = curso_service.listar_curso_id(id=id)
    ...
    def delete(self, id):
        ...
        if curso_bd is None:
            return make_response(jsonify('Curso não encontrado'), 404)
        curso_service.delete_curso(curso_bd)
        return make_response(jsonify('Curso deletado com sucesso!'), 204)
